...
** Debug
** AI pair programming
Use =elysium= to AI pair ~programming~. It uses gptel as the back end.

#+begin_src elisp
  (use-package elysium)
#+end_src

Use =smerge-mode= to then merge the changes.
...
* Social
** Mastodon
Open the Mastodon social network with =C-c j s m= key binding.
